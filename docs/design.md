...
> 当前实现说明。`docs/execution-plan-slim.md` 和 `docs/oldplan/` 是历史决策记录，不再作为实施基准。
...
**Layer 2 - LLM 深度反思（每 10 个完整问答轮次；未满时静默 40 分钟收尾）**：

将近期消息缓冲发给 LLM，返回小幅语义校正、心情判断、风格偏好更新和当前会话摘要候选。这是唯一的额外 LLM 调用点，用于规则无法覆盖的深层理解。

**Layer 3 - 结构化会话弧线（本地计算）**：按 60 分钟静默划分会话，记录状态差、峰值、转折点和规则 outcome；不按自然日切割，不做日终二次 LLM。

**Layer 4 - 互动画像证据**：只记录相处偏好和关系反应模式，带来源、证据和置信度；用户事实与长期记忆完全交给 LivingMemory。

### 3.4 收束机制：保留核心答案，降低主动性

传统插件都关注"如何让 Bot 回复"。CL 反其道而行之：**Bot 有权不回复**。

触发条件：
- `energy < 25`：低余裕收束，完整回答核心后减少铺陈和追问
- `boundary_pressure > 60`：边界收束，不主动延伸或推进关系
- `boundary_pressure >= 75`：强边界收束，只处理本轮必要事项

这里的“沉默”是历史命名，运行时不要求模型拒答。它只压缩寒暄、重复解释、额外建议、主动延伸和非必要追问，不得牺牲事实准确性、任务完成度或安全性，也不得诱导模型声称自己困倦或需要休息。

回复注入遵循：宿主系统与安全规则 > 本轮明确要求和边界 > 当前边界/安全约束 > 周期策略 > 关系基调 > 连续性 > 长期表达偏好。注入内容是不向用户复述的内部表达约束。
...
│   ├─ EventEngine.classify() → 类别 + 置信度 + 强度
│   ├─ StateEngine.apply_event() → 即时状态与趋势更新
│   ├─ storage.append_message() → 消息缓冲
│   └─ _maybe_trigger_reflection() → 每10轮立即触发；未满时静默40min收尾
│
├── 深度反思层 (LLM辅助)
│   ├─ 将近期消息缓冲发给LLM
│   ├─ 返回小幅语义校正、心情、风格偏好和会话摘要候选
│   └─ 更新CompanionState + StyleProfile，不决定会话 outcome
│
├── 会话弧线层 (本地规则)
│   ├─ 60分钟静默划分会话
│   ├─ 记录起止快照、峰值和转折点
│   └─ 计算规则 outcome 与跨会话连续性
│
├── 互动画像层 (证据驱动)
│   ├─ 明示偏好带来源、置信度和时间
│   └─ 高置信 active 证据才允许注入
