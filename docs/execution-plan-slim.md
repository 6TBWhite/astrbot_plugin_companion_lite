...
本文档取代 phase3 / phase4 / phase5 原计划中"全量方案"的执行地位。原计划文档保留作为完整设计参考，实际实现以本文档的瘦身版为准。
...
1. **P3 / P4（每日弧线 + 跨日连续性）是本插件真正不可替代的部分。** LLM 上下文窗口 + 记忆数据库能提供"近期聊了什么"和"过去的事实"，但天然覆盖不到"昨天以疲惫结束，今天该轻一点接"这种走势提炼。
