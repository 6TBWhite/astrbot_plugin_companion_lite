...
|---------|---------|--------------------------|----------------------------|---------------------|------------|------------|--------|---------|
| ≥69 | 很有精神 | "充足，很有精神" | —（走 closeness/bp 分支） | — | ×2.0 | ×0.0 | 正常扣 | -3/h 朝65 |
...
| ≤30 | 累了 | "很低，已经累了" | "低能量：温和简短，可结束话题" | —（>25 才触发） | ×0.3 | ×1.0 | **豁免** | +0.5/h 朝30 |
| <25 + 疲惫/低落 | — | — | — | **tired_low**（温柔简短） | ×0.3 | ×1.0 | **豁免** | +0.5/h 朝30 |
| <25 | — | — | — | **low_energy**（简短暗示休息） | ×0.3 | ×1.0 | **豁免** | +0.5/h 朝30 |

**关键边界**：
- **69**：甜点区上沿，"很有精神"短暂可见，自然演化快速拉回
...
| 22-39 | 谨慎 | "谨慎" | 由 closeness/energy 决定 | — | 阻断"放松"姿态（需<10） | 2.0/h | **注入**（≥35时） |
| 40-64 | 明显 | "防御" | "谨慎收敛：短而稳，减少玩笑" | — | "紧张"标签（≥40） | 2.0/h | 注入 |
...
|---------|------|---------|------------------|-----------|---------|---------|
| 0-7 | 刚认识（过早亲密触发区） | "刚认识" | 0~2.45 | 1.2 | ×1.0 | 不注入（≤15 才注入"很低"） |
| 8-14 | 刚认识 | "刚认识" | 2.8~4.9 | 1.2 | ×1.0 | ≤15 注入"熟悉度很低" |
| 15-24 | 认识 | "认识" | 5.25~8.4 | 0.7 | ×1.0 | 不注入 |
| 25-54 | 认识 | "认识" | 8.75~18.9 | 0.7 | ×1.0 | 不注入 |
| 55-64 | 熟人 | "熟人" | 19.25~22.4 | 0.25 | ×1.0 | 不注入 |
| 65-79 | 熟人 | "熟人" | 22.75~27.65 | 0.08 | ×0.65 | ≥45 注入"熟悉度较高" |
| 80-100 | 老朋友 | "熟人" | 28~35(封顶20) | 0.08 | ×0.35 → ×0.15 | ≥45 注入 |
