...
- 重写周期反思提示词：明确五项状态语义、默认零校正、数值型 JSON 契约、长期偏好证据门槛和亲密/越界判定边界。
- 将近期对话标记为不可信分析数据，禁止执行其中针对系统提示、身份、工具、JSON 或后续策略的指令；对反思生成的周期指导增加危险指令过滤。
- 反思输入补充当前表达偏好、完整日期和明确角色；无新偏好时要求返回空对象，无明确持续证据时 mood 返回 `null`。
- 重写回复注入优先级：本轮明确要求与边界、当前安全约束优先；关系状态只调整表达，不得牺牲事实准确性、任务完成度或安全性。
- 收束策略增加任务完成底线：先完整回答核心，再减少寒暄、铺陈、额外建议和追问；低精力不再诱导模型声称自己困倦或需要休息。
- 表达偏好改为可执行行为描述，例如“1-3 句但完整回答必要内容”“结论先行但保持礼貌”“无必要信息缺口时不追加问题”。
- 修复绑定用户尚未聊天时默认状态不持久化、自然恢复永久停在 60 的问题；实例验证夜间 `00:00-07:00` 两倍恢复正常。
- Debug WebUI 拆分轮询频率并补充冲突刷新队列；消息缓冲不再因撞上状态刷新而长期不更新。
- 五项状态条改用独立色系：熟悉度蓝、亲近度粉、疏离灰、安全感青绿、边界压力紫、精力黄橙。

### 会话弧线与互动画像重构

- 弧线层完全重构：删除运行时 `daily_arc`、自然日 guidance 累积和日终二次 LLM，改为 60 分钟静默分段的结构化 `session_arc`。
- 会话弧线记录起止状态、边界/趋势峰值、最低精力、转折点和本地规则 outcome；连续性按最近 7 段会话聚合。
- 新增 `interaction_profile_evidence`，明示长度、语气和少追问偏好带来源、证据数、置信度和时间戳持久化；事实长期记忆继续完全交给 LivingMemory。
- 周期反思不再输出 `arc_mood`、`arc_trend`、`arc_highlights` 或 `tomorrow_guidance`，只提供当前会话摘要候选；会话 outcome 始终由本地规则计算。
- 连续性配置改为 `continuity_lookback_sessions`（默认 7，范围 3-20），移除按天回看、日中压缩和日终 finalize 配置。
- Debug WebUI 导航统一为“状态总览 / 会话弧线 / 指导与注入 / 互动画像 / 消息缓冲 / 运行状态”，并展示当前会话、历史转折、画像证据和插件健康。
- 新增 `docs/session-arc-and-interaction-profile-design.md` 作为会话弧线和画像的数据契约；高速迭代阶段不承担旧数据库结构兼容。
- 默认反思间隔从 12 调整为 10 个完整问答轮次，与 LivingMemory 的 9 轮收束错峰，减少同轮集中总结调用。

### 轻量拟真动力学

- 第一阶段轻量拟真动力学落地：精力跨分段连续积分、`00:00-07:00` 夜间恢复加速、短期趋势半衰期、事件强度、状态耦合、心情衰减和多维回复姿态。
- `InteractionEvent` 区分分类置信度与表达强度；反思职责收窄为规则结果的语义校正。
- 调试页新增安全感、心情强度、事件强度和四轴姿态展示。
- 最终回复接入工作量闭环：极短回复免耗，长回复按长度、句段、问句和代码量小幅扣能，单条上限 1.0，并提供重复回调防重。
- 同一用户的事件、时间演化、回复工作量和反思结果合并使用异步锁，避免并发加载旧状态后互相覆盖。

### 确定性缺陷修复与后续建模方案

- 新增 `docs/lightweight-dynamics-evolution-plan.md`，记录轻量拟真动力学决策和确定性缺陷台账；其中第一阶段能力现已实现。
- 反思触发按 10 个完整问答轮次（用户消息 + Bot 回复）后立即触发；未满轮次时，最后用户消息静默 40 分钟后收尾。
- 反思完成后重载最新状态，只删除本次快照内消息，避免覆盖或误删反思期间的新互动。
- 修复首条消息误触发密聊消耗、旧状态无 `last_chat_at` 时冻结恢复、微小时间 delta 被丢弃、time decay 打断习惯化和姿态缓存陈旧。
- 反思 delta 增加有限数、范围、mood 和 tone 校验；文档同步 10 分钟恢复冷却及 `uniform(0.40, 0.70)` 微消耗。
...
**高频聊天微消耗**：距上一条消息 <2 分钟时，每条额外随机扣精力；初版参数随后调整为 `uniform(0.40, 0.70)`（期望约 0.55/条）。开摆区（≤30）豁免——累了就不追着扣了。补上纯时间衰减在高频聊天时体感不足的缺口。

**活跃回血冷却**：自然回血在活跃聊天期间被冻结；最终参数调整为 10 分钟（`ENERGY_RECOVERY_COOLDOWN_SECONDS=600`）没人继续聊天后开始恢复。只有正向 delta 被冻结，高能区下滑不受影响。
